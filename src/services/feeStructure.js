...
async function getStudentFeeSummary(coachingId, studentId) {
...
async function setStudentTotalFee({ coachingId, studentId, totalFee }) {
  ...
  const legacy = existing ? null : await getStudentFeeSummary(coachingId, studentId);
  ...
  await run(
    ...
    [coachingId, studentId, total, paidFee, pendingFee]
  );

  return getStudentFeeSummary(coachingId, studentId);
}

async function applyStudentPayment({ coachingId, studentId, amount }) {
  ...
  if (!existing) {
    const legacy = await getStudentFeeSummary(coachingId, studentId);
  ...
  await run(
    `INSERT INTO student_fee_structure (coaching_id, branch_id, student_id, total_fee, paid_fee, pending_fee, updated_at)
     ...
     DO UPDATE SET
       ...
       updated_at = CURRENT_TIMESTAMP`,
    [coachingId, studentId, paymentAmount, paymentAmount]
  );

  return getStudentFeeSummary(coachingId, studentId);
}

async function getNextDueDate(coachingId, studentId) {
  const row = await get(
